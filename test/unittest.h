   ...
   public:
    ...
    void run() override {
        try {
            ...
            method_();
            int failures =
                UnitTest::getInstance().getFailureNum() - old_failure_num;
            if (failures)
                std::cout << failures
                          << " failures are detected in the test case \""
                          << case_name_ << "\"" << std::endl;
...
int main() {
    signal(SIGSEGV, [](int) {
        UnitTest::getInstance().failure();
        std::cout << "unknown location(0): fatal error: received SIGSEGV."
                  << std::endl;
        UnitTest::getInstance().printLastCheckedPoint();
        report_and_exit();
    });
...
void do_check_failed(Msgs&&... msgs) {
    std::initializer_list<int> list{(std::cout << msgs, 0)...};
    std::cout << std::endl;
...
#define TEST_CASE(test_name)                                   \
    static void UnitTest_##test_name();                        \
    static TestCase UnitTest_##test_name##_case{               \
        UnitTest_##test_name, #test_name, __FILE__, __LINE__}; \
    static void UnitTest_##test_name()
    ...
    do {                                                                       \
        ...
        if (!(cond)) {                                                         \
            ...
            if (strict) {                                                      \
                ...
                std::cout << "critical error at " __FILE__ "(" << __LINE__     \
                          << ")." << std::endl;                                \
...
#define TEST_CHECK(cond, ...) G_CHECK(cond, false, __VA_ARGS__)

#define TEST_REQUIRE(cond, ...) G_CHECK(cond, true, __VA_ARGS__)
